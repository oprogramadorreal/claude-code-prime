...
Read `$CLAUDE_PLUGIN_ROOT/skills/init/references/agents/codebase-detection.md` for the full prompt template, detection algorithms, and return format for the Codebase Detection Agent.

Read these reference files and provide their content to the agent as context before the prompt:
...
Launch 1 `general-purpose` Agent tool call using the prompt from the codebase-detection.md file, prepended with the reference file contents above.
...
Read `$CLAUDE_PLUGIN_ROOT/skills/init/references/agents/documentation-audit.md` for the full prompt template, comparison dimensions, classification rules, and return format for the Documentation Audit Agent.

Launch 1 `general-purpose` Agent tool call using the prompt from the documentation-audit.md file. **Provide the Detection Results from Step 1 as context** at the start of the agent prompt (before the documentation-audit.md template content).
...
## Step 5b: Legacy Agent Cleanup

If `.claude/agents/code-simplifier.md` or `.claude/agents/test-guardian.md` exist, compare each against its template at `$CLAUDE_PLUGIN_ROOT/agents/`. If the content matches the template, silently remove it — these were installed by a previous plugin version but are no longer needed (agents now live at plugin level). If the content differs (user-customized), warn the user that the file has been modified and ask before removing. If `.claude/agents/` is empty after removal, remove the directory.

Also check `.claude/CLAUDE.md` for an `## Agents` section. If one exists, remove it entirely — this section was generated by a previous plugin version and is no longer used (agent prompts now live at plugin level). No user confirmation needed since this section was always plugin-generated content.
