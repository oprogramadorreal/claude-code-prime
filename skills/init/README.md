...
The main skill of the [optimus](https://github.com/oprogramadorreal/optimus-claude) plugin. Analyzes your project and sets up Claude Code for optimal performance by generating documentation and installing formatter hooks — all scoped to the project directory so they travel with the repo via git.
...
| `references/agents/` | Per-agent prompt templates for Codebase Detection and Documentation Audit subagents |
...
- **Agents**: `../../agents/` (code-simplifier, test-guardian) — plugin-level agent definitions
...
Note: re-running `/optimus:init` always overwrites `coding-guidelines.md` and hooks from the latest plugin templates — use `git diff` to review changes. Agents are now provided at the plugin level and always reflect the latest version. When the plugin version has increased since the last run, the audit also compares generated docs against current templates to detect improvements. To add project-specific rules that survive re-runs, put them in `.claude/CLAUDE.md` instead.
