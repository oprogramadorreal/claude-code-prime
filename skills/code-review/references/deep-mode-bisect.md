...
2. Snapshot the current state: `git stash push --include-untracked -m "pre-iteration-N-<baseline-ts>"` (using `--include-untracked` to capture new files, and `<baseline-ts>` is the timestamp from the baseline message for session-unique naming), then immediately restore: `git stash apply --index stash@{0}`. If the apply fails, abort deep mode immediately — drop the pre-iteration stash, then restore from the baseline: `REF=$(git stash list | grep -F '<baseline-message>' | head -1 | cut -d: -f1); if [ -z "$REF" ]; then echo "ERROR: baseline stash not found"; fi; git stash apply --index "$REF"`. Report the failure. If baseline restore also fails, instruct the user to run `git stash list` for manual recovery.
