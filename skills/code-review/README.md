...
- **Up to 7 parallel agents** — bug detection, security/logic, guideline compliance (x2 for cross-validation), code-simplifier, contracts-reviewer (when API-related files are in the diff), and test-guardian (when test infrastructure is detected)
...
4. Launches up to 7 parallel review agents (bug detection, security/logic, guideline compliance x2, code-simplifier, contracts-reviewer, test-guardian)
...
| Agent types | 2 CLAUDE.md compliance + 1 bug + 1 security | 2 guideline compliance + 1 bug + 1 security + code-simplifier + contracts-reviewer + test-guardian |
...
| Agents | Up to 7 parallel (bug, security, guidelines x2, simplifier, contracts, test-guardian) | Up to 4 parallel (guideline compliance, testability, duplication/consistency, code simplifier) |
...
- Git
- Project initialized with `/optimus:init` (recommended, not required — enables all agents and project-specific guidelines)
