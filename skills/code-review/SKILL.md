...
Agent 5 (Code Simplifier) always runs. Agent 6 (Test Guardian) runs only if test infrastructure is detected (`.claude/docs/testing.md` exists, or any `docs/testing.md` in monorepo subprojects — use the same detection as Step 2's doc loading).

If no test infrastructure is detected, Agent 6 is skipped — only Agents 1–5 run.
...
- Agents available (5 always; Agent 6 if test infrastructure detected)
...
4 core review agents + 2 quality agents, all launched in parallel for maximum coverage.

Launch up to 6 `general-purpose` Agent tool calls simultaneously. Agents 1–5 always run; Agent 6 only runs if test infrastructure is detected (checked in Step 2).

Each agent receives the list of changed file paths from Step 1.

Read `$CLAUDE_PLUGIN_ROOT/skills/code-review/references/agents/shared-constraints.md` for quality bar, exclusion rules, and false positive guidance that apply to all agents.

For each agent, read its prompt template from `$CLAUDE_PLUGIN_ROOT/skills/code-review/references/agents/`:
- Agent 1: `agent-1-bug-detector.md`
- Agent 2: `agent-2-security-logic-reviewer.md`
- Agents 3–4: `agent-3-4-guideline-compliance.md` (both agents use the same prompt template)
- Agent 5: `agent-5-code-simplifier.md`
- Agent 6: `agent-6-test-guardian.md`

### PR/MR context injection (PR/MR mode only)

If a `pr-description` was captured in Step 1 and its body is non-empty, prepend the PR/MR context block to every agent prompt before the file list. Read the **PR/MR Context Block** section in `$CLAUDE_PLUGIN_ROOT/skills/code-review/references/agents/shared-constraints.md` for the template, truncation rule, and guardrail language.

### Iteration context injection (deep mode, iterations 2+)

If deep mode is active and `iteration-count` > 1, prepend the iteration context block to every agent prompt before the file list (after the PR/MR context block, if present). Read the **Iteration Context Block** section in `$CLAUDE_PLUGIN_ROOT/skills/code-review/references/agents/shared-constraints.md` for the template and format.
...
| 5 — Code Simplifier | Unnecessary complexity, naming, dead code, pattern violations | Always |
| 6 — Test Guardian | Test coverage gaps, structural barriers to testability | Test infrastructure detected |
...
**Agent availability summary**: Agents 1–5 always run (no project dependencies). Agent 6 requires test infrastructure. If test infrastructure is missing, recommend `/optimus:init` for test setup.
