...
Read `$CLAUDE_PLUGIN_ROOT/skills/dev-setup/agents/project-detector.md` for the full prompt template, detection tasks, and return format for the Project Context Detection Agent.
...
Launch 1 `general-purpose` Agent tool call using the prompt from project-detector.md, prepended with the shared constraints and reference file contents above.
...
Read `$CLAUDE_PLUGIN_ROOT/skills/dev-setup/agents/instructions-auditor.md` for the full prompt template, audit tasks, and return format for the Dev Instructions Audit Agent.
...
Launch 1 `general-purpose` Agent tool call using the prompt from instructions-auditor.md. **Provide the Context Detection Results from Step 1 as context** at the start of the agent prompt (before the readme-section-detection.md content and agent template).
