...
Read `$CLAUDE_PLUGIN_ROOT/skills/dev-setup/references/agents/project-context-detection.md` for the full prompt template (Constraints + Prompt), detection tasks, and return format for the Project Context Detection Agent.
...
Launch 1 `general-purpose` Agent tool call using the prompt from the project-context-detection.md file, prepended with the reference file contents above.
...
Read `$CLAUDE_PLUGIN_ROOT/skills/dev-setup/references/agents/dev-instructions-audit.md` for the full prompt template (Constraints + Prompt), audit tasks, and return format for the Dev Instructions Audit Agent.
...
Launch 1 `general-purpose` Agent tool call using the prompt from the dev-instructions-audit.md file. **Provide the Context Detection Results from Step 1 as context** at the start of the agent prompt (before the readme-section-detection.md content and the audit prompt).
