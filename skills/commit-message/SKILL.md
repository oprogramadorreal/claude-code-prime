...
When the feature is ready, recommend running `/optimus:pr` to create a pull request, or `/optimus:verify` to validate the feature end-to-end. For best results, run the next skill in a fresh conversation — each skill gathers its own context from scratch.
