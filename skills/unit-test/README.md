...
Well-maintained code has [30%+ fewer AI-introduced defects](https://arxiv.org/abs/2601.02200), and tests are what make AI agents self-correcting: make change → run tests → see failure → fix. `/bootstrap:init` installs a test-guardian agent that monitors coverage gaps, but it doesn't write tests. `/bootstrap:unit-test` is the active complement: it fills gaps deliberately.

**Conservative by design** — only adds new test files, never refactors or restructures existing source code. If code is untestable as-is, it flags it rather than changing it. Refactoring is the domain of `/bootstrap:simplify`.

## Features

- **Pre-flight check** — verifies `/bootstrap:init` has been run and identifies available guideline documents
- **Project-wide discovery** — scans for test files, frameworks, coverage tooling, and bootstrap infrastructure status
...
- `/bootstrap:unit-test` — full project test coverage improvement
- `/bootstrap:unit-test src/api` — scope to a specific directory
- `/bootstrap:unit-test packages/auth` — scope to a monorepo subproject
...
- **After `/bootstrap:init`** — establish test coverage for a newly bootstrapped project
...
4. Provisions bootstrap infrastructure (test-guardian, testing.md, CLAUDE.md updates)
...
| | Test-guardian agent | `/bootstrap:unit-test` |
|---|---|---|
